...
class SmartSchedulingService {
     ...
     */
    async notifyConflictsForNewManualTask(task) {
        if (!task || task.isAutoSchedule)
            return false;
        ...
        const overlappingEvents = await models_1.ScheduleItem.find({
            userId: task.userId,
            type: 'event',
            startTime: { $lt: end },
            endTime: { $gt: start }
        ...
        for (const ev of overlappingEvents) {
            const bufferedStart = (0, date_fns_1.addMinutes)(new Date(ev.startTime), -10);
            const bufferedEnd = (0, date_fns_1.addMinutes)(new Date(ev.endTime), 10);
            const overlapsBuffered = start < bufferedEnd && end > bufferedStart;
            if (overlapsBuffered) {
                conflictFound = true;
                await this.notificationService.sendNotification(notificationService_1.NotificationType.EVENT_CONFLICT, task.userId, {
        ...
        const overlappingManualTasks = await models_1.Task.find({
            userId: task.userId,
        ...
        for (const mt of overlappingManualTasks) {
            conflictFound = true;
            await this.notificationService.sendNotification(notificationService_1.NotificationType.TASK_CONFLICT, task.userId, {
                taskId: task.id || task._id?.toString(),
                taskTitle: task.title,
    ...
    async getScheduleItemsWithBuffer(userId, targetDate, task, excludeTaskIds = []) {
        ...
        if (targetDate) {
            ...
            const windowStart = (0, date_fns_1.startOfDay)(targetDate);
            ...
            let endOfSearchWindow = task.endTime ? new Date(task.endTime) : (0, date_fns_1.addDays)(targetDate, 7);
            if (!endOfSearchWindow || endOfSearchWindow <= targetDate) {
                endOfSearchWindow = (0, date_fns_1.addDays)(targetDate, 7);
            }
    ...
    async updateTaskWithRescheduling(taskId, updates) {
        ...
        if (updates.startTime && updates.estimatedDuration) {
            ...
            updates.endTime = (0, date_fns_1.addMinutes)(new Date(task.startTime), updates.estimatedDuration);
        }
        // Apply updates
        ...
        if (task.isAutoSchedule) {
            ...
            if (task.isDeadlineConstrained && task.originalDeadline) {
                const hours = Math.max(0, (0, date_fns_1.differenceInHours)(new Date(task.originalDeadline), new Date()));
        ...
        if (task.startTime && task.endTime) {
            await models_1.ScheduleItem.findOneAndUpdate({ taskId: task.id || task._id?.toString() }, { startTime: task.startTime, endTime: task.endTime }, { upsert: true });
        }
     ...
     */
    async rescheduleTasksForNewManualTask(manualTask) {
        if (!manualTask.startTime || !manualTask.endTime)
            return;
        ...
        for (const t of conflictingTasks) {
            ...
            if (newTime) {
                ...
                overlapsAny = daySchedule.some(item => {
                    // Skip self by taskId
                    if (item.type === 'task' && item.id && (item.id === (t.id || t._id?.toString())))
                        return false;
                    return newTime.startTime < item.endTime && newTime.endTime > item.startTime;
                });
            }
            if (newTime && !overlapsManual && !overlapsAny) {
                const oldTime = { startTime: t.startTime, endTime: t.endTime };
                t.startTime = newTime.startTime;
                t.endTime = newTime.endTime;
                await t.save();
                await models_1.ScheduleItem.findOneAndUpdate({ taskId: t._id?.toString() }, { startTime: t.startTime, endTime: t.endTime });
                await this.notificationService.sendNotification(notificationService_1.NotificationType.TASK_RESCHEDULED, t.userId, {
                    taskTitle: t.title,
                    oldTime,
                    newTime,
                ...
                await this.notificationService.sendNotification(notificationService_1.NotificationType.NO_OPTIMAL_TIME, t.userId, {
                    taskTitle: t.title,
    ...
    async rescheduleTasksForNewEvent(event) {
        if (!event.startTime || !event.endTime)
            return;
        const bufferedEventStart = (0, date_fns_1.addMinutes)(new Date(event.startTime), -10);
        const bufferedEventEnd = (0, date_fns_1.addMinutes)(new Date(event.endTime), 10);
        ...
        for (const t of conflictingTasks) {
            ...
            if (newTime && !overlapsEvent && !overlapsAny) {
                ...
                t.endTime = newTime.endTime;
                await t.save();
                await models_1.ScheduleItem.findOneAndUpdate({ taskId: t.id || t._id?.toString() }, { startTime: t.startTime, endTime: t.endTime });
                await this.notificationService.sendNotification(notificationService_1.NotificationType.TASK_RESCHEDULED, t.userId, {
                    taskTitle: t.title,
                    oldTime,
                    newTime,
